...
- **신호 수명주기 이탈**: §5는 리밸일 당일 저녁에도 마지막 신호가 파일에 남는다고
  했으나, 구현은 비경계 밤마다 파일을 빈 상태로 재작성한다 (비활성화 정리의
  단순성 우선). 따라서 "지난 신호(is_stale)" 표시는 휴장 근사로 신호가 일찍 뜬
  경우와 배치 실패로 파일이 낡은 경우에만 나타난다.
