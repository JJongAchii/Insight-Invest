...
**범위 축소 3건** — 모두 Phase 1 API 계약(`GET /backtest/strategy/analytics/{port_id}`,
live 응답 확장)이 이미 확정된 뒤에 프론트를 붙이면서 드러났다. 셋 다 서버 쪽에
계약 밖 재료(신규 필드·신규 계산)가 필요해 이번 Phase 2 범위에서 뺐다 — 후속으로
넘긴다:

- **④ NAV 차트의 벤치마크 라이브 연장** — ~~후속~~ **2026-08-05 해소**: live
  응답에 `bm_live`(저장 후 SPY, 첫 관측=1000 리베이스 + 같은 구간 지표)를 추가해
  차트 BM 라인이 저장 후까지 이어지고, 지표 비교 표에 "BM (저장 후)" 열이 생겼다.
  범례·표가 벤치마크 실체(SPY)를 명시한다.
- **⑤ 운영 구간 리밸 이력** — 설계상 `live_weights`에서 뽑은 진입/이탈/구간수익
  타임라인(운영 구간) + 백테스트 구간 타임라인을 나란히 보여줄 예정이었다.
  `NextRebalCard`는 "다음" 신호만 보여줄 뿐, 과거 라이브 리밸 이력을 집계해
  주는 엔드포인트가 없어 이번엔 미구현.
- **② 실보유 vs 목표 배분의 괴리(deviation) 열** — `HoldingsNowCard`는
  `live_weights`(어제, 드리프트) 비중만 보여준다. "현 주기 목표 비중"과의
  괴리 계산은 목표 비중을 라이브 시점 기준으로 다시 조회하는 서버 재료가
  필요해 후속으로 넘겼다. 원 설계 의도대로 "백테스트 최종 배분"은
  `MetricSummary` 라벨로 명시적으로 강등해 실보유와 혼동되지 않게 했다(§1 ②
  후반부는 충족).

**Task 3에서 확인된 것** — live NAV의 앵커 행 규약이 여러 컴포넌트에 걸쳐
일관되게 적용됐다:
- `liveNav[0]`은 저장일(saved_at) 앵커 행(~1000 base)이지 경과 거래일이
  아니다. 따라서 "경과 거래일" 정의는 `len(live_nav) - 1`이다
  (`LiveHeadline.tsx`의 `elapsedDays`) — 서버 `live_percentile`의 `n_days`
  계산과 동일 규약이라 `LiveHeadline`과 `ExpectationCard`의 "{n_days}거래일"
  표기가 서로 어긋나지 않는다.
- 라이브 기간의 구간 수익(연/월 병기, `buildReturnData`)은 이 앵커 행을
  `initialRef`로 넘겨 계산한다 — 저장 후 첫 (부분) 기간도 앵커 대비 수익률로
  막대가 생기고, 두 번째 기간부터 생기지 않는다.
