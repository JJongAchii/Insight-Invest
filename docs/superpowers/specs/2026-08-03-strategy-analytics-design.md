...
- **④ NAV 차트의 벤치마크 라이브 연장** — `LineChart`는 저장 후 전략 NAV는
  라이브 시리즈로 연장하지만, 벤치마크는 백테스트 구간의 정적 `bm_nav`만 그린다.
  라이브 구간 벤치마크를 그리려면 서버가 저장일 이후 벤치마크 가격을 별도로
  내려줘야 하는데 현재 live 응답 계약엔 없다.
