...
- 상태: **구현 완료·프로덕션 발행 시작** (2026-08-14, main 1c381fc 머지·배포, §14 확인 기록)
...
## 14. 구현 확인 기록 (2026-08-14)

- SDD 7태스크 전부 개별 리뷰 통과, 최종 전체 리뷰(상위 모델) 후 수정 웨이브 1회.
  main 1c381fc fast-forward 머지, 서버 테스트 180개·클라 lint/tsc 클린.
- **최종 리뷰가 잡은 Critical 2건** (개별 태스크 리뷰로는 안 보이던 크로스커팅):
  1. 한국판 피드 URL(`hl=ko-KR`)은 302 정규화 리다이렉트 — 기존 라이브 클라이언트가
     `follow_redirects` 미설정이라 KR 라이브 헤드라인이 항상 빈 목록이 될 뻔. 1줄 수정.
  2. `run_pipeline.sh`가 매 실행 `: > .env` 후 SSM 고정 목록만 복원 —
     §10 초안의 ".env 직접 추가" 절차는 다음 실행 때 키가 지워지는 죽은 경로였다.
     SSM KEY 루프에 `ANTHROPIC_API_KEY` 추가(quant-data 0274d28) + §10을 SSM 절차로 정정.
     키는 SecureString `/qdata/ANTHROPIC_API_KEY` v1로 등록됨(값 비노출 서브셸 등록).
  3. (Important) `import anthropic` 실패가 발행 전체를 막던 사각지대 → try/except 폴백.
- **첫 실발행 검증** (로컬 수동 1회, 16:08 KST): `10건, curated=True, edition=evening` —
  Haiku 큐레이션 실작동, 종합 5(데드크로스·특검 기소·SMR 동맹·北해커 보안·드론 관세)
  + 경제 5(LS 최대 실적·LG 휴머노이드·강남 아파트 첫 하락·美 금리동결 베팅·하이닉스
  중복상장 논란), 전 항목 why 포함, 클러스터 5개사.
