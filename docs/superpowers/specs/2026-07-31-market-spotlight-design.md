...
- **관리종목 미필터** — 거래정지·무거래일은 §15.2로 제외되지만, 관리종목
  지정 자체는 거르지 않는다.
...
### 15.2 거래정지·무거래일 제외 — 두 번째 유니버스 정정

운영 첫 테스트 발송(2026-07-31)에서 신고가 유지 그룹 1위가 8거래일 연속
거래량 0인 거래정지 종목이었다. 정지 종목은 가격이 동결돼 조건을 공짜로
만족하고, 시장 급락기에는 살아있는 종목이 다 빠지는 동안 정지 종목만 남아
그룹을 지배한다.

정정: 스팩과 동일하게 **선정·통계 양쪽**에서 당일 거래량 0(무거래·정지·
미상장) 종목-일을 제외한다. 경제적 근거 — 거래할 수 없는 날의 신호는 실행
불가능하다. 선정 쪽은 `module/spotlight.active_tickers`(당일 거래량 > 0),
통계 쪽은 `liquid &= (V > 0)`.

격리 효과 (같은 7/20 레이크, near_52w_high_hold, 기준선 대비):

| 지평선 | Δ중앙값 (필터 전 → 후) | Δ승률 (전 → 후) |
|---|---|---|
| 5일 | +0.115%p → **−0.006%p** | +1.63 → +2.09%p |
| 20일 | +0.447%p → **+0.133%p** | +2.91 → +3.04%p |
| 60일 | +0.787%p → **+0.222%p** | +2.18 → +2.64%p |

N(20일)은 259,530 → 188,846 — 모집단의 ~27%가 무거래일이었고, 그 동결
가격(전방수익 ≈0)이 음수 중앙값 기준선 대비 '양의 우위'로 둔갑해 있었다.
정직한 잔여 우위는 중앙값이 아니라 **승률(+3.0%p)과 평균(우상향 꼬리,
20일 mean_excess +1.04%p)**에 있다. 실측치 병기 원칙은 그대로 — 무엇을
보여줄지는 통계가 아니라 사용자가 판단한다.
