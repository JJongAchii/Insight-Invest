...
- 월 $2는 **이 Research 작업의 UTC 월별 요청 예약 상한**이다. OpenAI 계정 전체,
  ...
  2026-09-11 사용자 재배분 승인에 따라 운영 $1.40 + 인수 시험 $0.60으로 분리하여
  이 둘을 합해 월 $2 안에 둔다. 이전 $1.50/$0.50 분배와 실패 예약 기록은 아래에 보존한다.
  시험 workflow는 실제 운영 함수의 $1.40 설정을 읽어 확인한 뒤에만 호출한다.
