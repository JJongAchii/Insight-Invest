...
- 월 $2는 **이 Research 작업의 UTC 월별 요청 예약 상한**이다. OpenAI 계정 전체,
  ...
  2026-09-14 사용자의 검증 우선 예산 사용 승인에 따라 09-15 운영 $0.90 + 인수 시험
  $1.10으로 분리하여 이 둘을 합해 월 $2 안에 둔다. 이전 $1.00/$1.00,
  $1.40/$0.60 및 $1.50/$0.50 분배와
  실패 예약은 삭제하거나 초기화하지 않는다. 추가 월 총액 증액은 별도 승인을 받는다.
  시험 workflow는 실제 운영 함수의 $0.90 설정을 읽어 확인한 뒤에만 호출한다.
