...
2026-09-10: 키 연결과 원문 우선 main/AWS/Vercel 배포는 완료했다. 사용자는 이어서
읽기 가치·한국어 요약·과거 글 재알림 방지 개선을 승인했다. GPT-5 mini/medium과 기존
예산을 유지한다. v8 생성/v3 검수의 고정 표본 시험에서도 의미 오류와 시장 해설 오분류가
남았으므로 **자동 요약·핵심 승격·알림 인수는 미통과**다. 원문 우선 운영과 요약 활성화를
구분하며, ResearchAnalysisEnabled는 계속 false로 유지한다.
...
- 그보다 먼저 `ResearchAnalysisEnabled=false`이면 `editorial_release_pending`으로
  반환한다. 이 기본 상태는 키가 있어도 API 비용을 발생시키지 않으며, 캐시된 요약도
  핵심/알림으로 노출하지 않는다. 읽기 API와 폴러에 같은 스위치를 적용한다.
  의미 품질 인수 전에는 `true`로 바꾸지 않는다. 원문 우선 배포도 별도 릴리스 검사가 필요하다.
...
- 입력 24,000자, 정기 실행당 최대 1회 유료 단계(생성 또는 검수), 문서/단계 버전당 최대 3회 시도. 30분 재시도 간격과
...
위 1의 키/연결 확인과 원문 우선 운영 배포는 완료했다. 2의 실제 요약 품질 인수와
자동 요약/핵심 알림 활성화는 미완료다. Actions 34179045960과 34185928307이
