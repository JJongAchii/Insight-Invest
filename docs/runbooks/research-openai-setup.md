...
2026-09-10 릴리스 준비: 기존 v8/v3 실패 기록은 아래에 보존한다. 사용자의 후속
본문 추출·요약 진행 요청에 따라 source-only 선별(v2) → 인용별 한국어 초안(v11)
→ 별도 원문 검수(v8)로 분리했다. GPT-5 mini/medium과 기존 월 예산을 유지한다.
...
현재 고정 6개 출처 표본은 초기 제한 운영의 검토 범위다. 아래 20–30편의 폭넓은
품질 인수를 완료한 것으로 주장하지 않는다. 현재 표본에서도 읽을 수 있는 원문 선별과
