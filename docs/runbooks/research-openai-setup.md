...
2026-09-09: 사용자가 키 등록을 완료했고 GitHub secret 이름을 확인했다.
실제 nano/mini 비교 후 사용자가 근거·분류·한국어 보완과 mini 반영을 승인했다.
작업 브랜치 기본 모델은 GPT-5 mini다. 최종 세 편 실제 재시험까지 실행했지만 원문 대조에서
의미 오류가 남아 인수 미통과다. v2/medium 검수도 AQR 용어 오류를 놓쳤다.
요약 릴리스 스위치는 기본 비활성이다. main/AWS/Vercel 운영 배포는 하지 않았다.
...
현재 브랜치는 mini, `reasoning.effort=medium`, v7 한국어 편집 및 분리 인용 계약을 사용한다.
