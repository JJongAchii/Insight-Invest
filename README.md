...
iPhone Safari와 홈 화면 PWA에서는 페이지 최상단에서 아래로 당기면 현재 URL을 유지한 채
화면을 다시 불러온다. `Pull to refresh`가 `Release to refresh`로 바뀐 뒤 손을 놓아야
실행되며, 짧은 당김·가로 스와이프·본문 중간 스크롤·입력 또는 내부 스크롤 영역에서는
작동하지 않는다.
