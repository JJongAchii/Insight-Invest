...
def update_daily_price(market: str):
    ...
    for start_idx in range(0, num_records, batch_size):
        ...
        if batch_last_price["max_dt"].isna().any():
            ...
            new_data = pd.merge(
                price, batch_last_price[["meta_id", "ticker"]], on="ticker", how="left"
            )
        ...
        for date in unique_dt:
            ticker_list = batch_last_price.loc[
                batch_last_price["max_dt"] == date, "ticker"
            ].tolist()
            ...
            mg_returns = returns.mul(batch_last_price.set_index("ticker")["adj_close"].squeeze())
            ...
            mg_close = pd.merge(mg_returns, cl_price, on=["trade_date", "ticker"], how="left")

            mg_gross = pd.merge(mg_close, gross_return, on=["trade_date", "ticker"], how="left")

            mg_data = pd.merge(
                mg_gross, batch_last_price[["meta_id", "ticker"]], on="ticker", how="left"
            )
