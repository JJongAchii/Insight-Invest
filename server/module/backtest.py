...
class Backtest:
    ...
    def data(
        self, meta_id: Union[int, List] = None, tickers: Union[str, List] = None, source: str = "db"
    ) -> pd.DataFrame:
        ...
        if source == "db":
            with db.session_local() as session:
                ...
                ).join(db.TbPrice, db.TbPrice.meta_id == db.TbMeta.meta_id)
                if meta_id:
                ...
                data = db.read_sql_query(query=query, parse_dates="trade_date").pivot(
